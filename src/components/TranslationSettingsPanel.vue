...
function enableBasic(enabled: boolean) {
  ...
  if (enabled) advanced.enabled = false;
}

function enableAdvanced(enabled: boolean) {
  advanced.enabled = enabled;
  if (enabled) basic.enabled = false;
}
...
<template>
  <div class="translation-settings">
    <div class="page-intro">
      <div><h2>{{ text("翻译配置", "Translation") }}</h2><p>{{ text("预览优先读取本地译文；未命中时使用当前启用的引擎翻译并保存到本地。", "Previews use a local translation first, then translate with the enabled engine and cache the result locally.") }}</p></div>
      <span class="stage-badge">{{ text("配置接入", "Configuration") }}</span>
    </div>

    <div v-if="loading" class="state-card">{{ text("正在读取配置…", "Loading settings…") }}</div>
    <template v-else>
      <section class="config-card" :class="{ disabled: !basic.enabled }">
        <div class="card-heading"><div><span class="eyebrow">{{ text("专用引擎", "DEDICATED") }}</span><h3>{{ text("专用翻译 API", "Translation API") }}</h3><p>{{ text("适合稳定、低成本的日常文本翻译。", "For predictable, low-cost everyday translation.") }}</p></div><label class="engine-switch"><input type="checkbox" :checked="basic.enabled" @change="enableBasic(($event.target as HTMLInputElement).checked)" /><span>{{ basic.enabled ? text("已启用", "Enabled") : text("未启用", "Disabled") }}</span></label></div>
        <div class="engine-state"><span class="status" :class="{ ready: basicConfigured }">{{ basicConfigured ? text("密钥已就绪", "Key ready") : text("未配置密钥", "No key") }}</span></div>
        <div class="form-grid">
      ...
      <section class="config-card advanced-card" :class="{ disabled: !advanced.enabled }">
      ...
      <div class="security-note"><strong>{{ text("本地优先与密钥安全", "Local first and key safety") }}</strong><span>{{ text("两类引擎只能启用一种。API Key 不写入磁盘；翻译结果仅保存在本地 .metadata/translations 目录。配置保存到：", "Only one engine can be enabled. API keys are not written to disk; translations stay in the local .metadata/translations directory. Settings are saved to:") }} {{ configPath || "%USERPROFILE%\\Skill Manager\\.metadata\\translation-settings.json" }}</span></div>
